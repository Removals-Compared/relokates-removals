...
// Supabase (leads table) + Resend (email) + HubSpot (CRM)
...
export default async function handler(req, res) {
  ...
  try {
    await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'Relokates Quotes <quotes@relokates.co.uk>',
  ...
  // ── 4. HUBSPOT CRM ──
  try {
    // Build note with all removal details for HubSpot timeline
    const noteBody = [
      `Service: ${service || 'Not specified'}`,
      `Moving from: ${move_from || 'Not specified'}`,
      `Moving to: ${move_to || 'Not specified'}`,
      `Move date: ${move_date || 'Not specified'}`,
      `Property size: ${property || 'Not specified'}`,
    ...
    const hsRes = await fetch('https://api.hubapi.com/crm/v3/objects/contacts', {
      ...
      body: JSON.stringify({
        properties: {
          ...
          hs_lead_status: 'NEW',
          lifecyclestage: 'lead',
          website: 'relokates.co.uk',
          hs_content_membership_notes: noteBody
        }
      })
    });
    ...
    if (!hsRes.ok) {
      ...
      if (hsRes.status === 409) {
        ...
        if (emailSearch.ok) {
          const existing = await emailSearch.json();
          await fetch(`https://api.hubapi.com/crm/v3/objects/contacts/${existing.id}`, {
            ...
            body: JSON.stringify({
              properties: { hs_lead_status: 'NEW', phone, hs_content_membership_notes: noteBody }
            })
          });
        }
      } else {
