...
- **Lead tagging**: the quote form on `/removals-birmingham` sets `branch: "birmingham"`, which `/api/quote` whitelists and stores as `source = relokates.co.uk/birmingham` (no schema change needed). Branch notification email: set `BIRMINGHAM_NOTIFY_EMAIL` in Vercel when the branch owner's address is confirmed — until then branch lead alerts go to info@relokates.co.uk. Customer auto-replies for Birmingham leads show the branch phone (07947 229838).
